...
def run_stable_analysis(tickers):
    ...
    formatted_tickers = [f"{t.strip().replace(',', '')}.NS" if not str(t).strip().replace(',', '').endswith(".NS") else str(t).strip().replace(',', '') for t in tickers]
    ticker_map = {sym: sym.replace(".NS", "") for sym in formatted_tickers}
    ...
    def evaluate_ticker_regime(sym):
        try:
            ...
            filter_passed, active_regime, entry_level, reason = run_vsa_hmm_engine(d_df)
    ...
    with ThreadPoolExecutor(max_workers=10) as executor:
        futures = [executor.submit(evaluate_ticker_regime, sym) for sym in formatted_tickers]
