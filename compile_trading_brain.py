...
def compile_playbook_rules(closed_trades):
    ...
    lines = [
        ...
        "",
        "### Invariant E: The 10-Session Re-Entry Quarantine (Anti-Boomerang Rule)",
        "- **Context:** Derived from `HFCL` (Round 2: -7.0% loss wiped out Round 1 +4.3% profit), `HONASA` (-6.2% exit re-bought 4 days later into another -2.9% drawdown), and `FEDERALBNK` re-entry into immediate breach.",
        "- **Rule:** When a ticker is exited (profit or loss), place it in a mandatory 10 trading session quarantine. Never re-enter the same ticker within 10 sessions, and NEVER chase it at a higher price than the exit price without a minimum 3-week base.",
        "",
        "### Invariant F: The 45-Day Compounding Law (Power Law Alpha Distribution)",
        "- **Context:** Validated by 22-trade audit: Holds >= 45 days yielded 100% win rate (10/10 wins) and generated +₹3,30,701.40 (98.4% of all realized gains). Holds < 15 days yielded 0% win rate (0/4 wins, -₹41,901.88 drag).",
        "- **Rule:** High-conviction setups respecting the 20-Day SMA and trailing ratchets must not be trimmed or exited on single intraday red candles or minor noise. The system's wealth is created between Day 45 and Day 110.",
        "",
        "---",
